...
def get_model_added_space_heater():
    model = tb.Model(id="model_1st_floor_added_space_heater", saveSimulationResult=True)
    filename = os.path.join(uppath(os.path.abspath(__file__), 1), "configuration_template_DP37_full_no_cooling_ano1_added_space_heater.xlsm")
...
def get_model_cut_added_space_heater():
...
def get_model_cut_space_heater():
    model = tb.Model(id="cut_space_heater", saveSimulationResult=True)
    filename = os.path.join(uppath(os.path.abspath(__file__), 1), "cut_space_heater.xlsm")
    model.load_model(semantic_model_filename=filename, infer_connections=True, fcn=fcn, create_signature_graphs=False, validate_model=False)
    return model
...
def run():
    ...
    endTime = datetime.datetime(year=2023, month=12, day=12, hour=0, minute=0, second=0,
                                tzinfo=gettz("Europe/Copenhagen"))"""
    model = get_model_cut_added_space_heater()
    # model = get_model_cut_space_heater()
